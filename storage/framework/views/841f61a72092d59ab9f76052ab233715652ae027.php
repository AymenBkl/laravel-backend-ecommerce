...
<script src="<?php echo asset('web/js/jquery.instagramFeed.min.js'); ?>"></script>
...
<?php endif; ?>
<script src="<?php echo asset('web/js/scripts.js'); ?>"></script>
<script>
/////////////////// default code ///////////////////
...
jQuery(document).ready(function($){
	jQuery.instagramFeed({
		'username': "<?php echo e($result['commonContent']['settings']['instauserid']); ?>",
...
var url = form.attr('action');
  jQuery.ajax({
        ...
        type: "GET",
        url: "<?php echo e(url('/subscribeMail')); ?>",
        ...
        success: function(data)
        {
          ...
          if(data == '1'){
              jQuery('.email').val('');
              notificationWishlist("<?php echo app('translator')->get('website.mailchimp_subscribe'); ?>");
          }else if(data == '2'){
              notificationWishlist("<?php echo app('translator')->get('website.mailchimp_already_subscribe'); ?>");
          }
          else{
              notificationWishlist("<?php echo app('translator')->get('website.mailchimp_error'); ?>");
          }
        }
      });
...
jQuery(document).on('click','#allow-cookies', function(e){
	jQuery.ajax({
		headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
		url: '<?php echo e(URL::to("/setcookie")); ?>',
		type: "get",
		...
		jQuery.ajax({
			...
			success: function (res) {
				if(res.status == 'exceed'){
					notificationWishlist("<?php echo app('translator')->get('website.Ops! Product is available in stock But Not Active For Sale. Please contact to the admin'); ?>");
				}
				else{
					...
					jQuery.ajax({
						...
						url: '<?php echo e(URL::to("/addToCartFixed")); ?>',
						headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
					...
					jQuery.ajax({
						...
						url: '<?php echo e(URL::to("/addToCartResponsive")); ?>',
						headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
					...
					notificationWishlist("<?php echo app('translator')->get('website.Product Added Successfully'); ?>");
	...
	jQuery.ajax({
		...
		success: function (res) {
			...
			if(res.remainingStock>0){
				jQuery('#min_max_setting').html("&nbsp; <?php echo app('translator')->get('website.Min Order Limit:'); ?>" + min_level);
		 ...
		 if(res['status'] == 'exceed'){
		   notificationWishlist("<?php echo app('translator')->get('website.Ops! Product is available in stock But Not Active For Sale. Please contact to the admin'); ?>");
			...
			headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
		   	 ...
		   	 notificationWishlist("<?php echo app('translator')->get('website.Product Added Successfully Thanks.Continue Shopping'); ?>");
  ...
  jQuery.ajax({
        ...
        success: function(data)
        {
          if(data == 'done'){
              ...
              notificationWishlist("<?php echo app('translator')->get('website.Thanks For Your Time And Considration For Providing FeedBack For This Product'); ?>");
          }else if(data == 'not_login'){
              notificationWishlist("<?php echo app('translator')->get('website.In Order To Give FeedBack You Have Must Login First. Thanks'); ?>");

          }
          else{
              notificationWishlist("<?php echo app('translator')->get('website.You Have Already Given The Comment. Thanks'); ?>");
...
function paymentMethods(){
	...
	//jQuery(document).on('click', '#midtrans_button', function(e){
		...
		if(payment_method == 'midtrans'){
			...
			jQuery.ajax({
				headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
				url: '<?php echo e(URL::to("/midtrans/transaction")); ?>',
				type: "get",
				success: function (res) {
					jQuery('#loader').hide();
...
jQuery(document).on('click', '#paystack_button', function(e){
	...
	jQuery.ajax({
		headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
		url: '<?php echo e(URL::to("/paystack/transaction")); ?>',
		type: "get",
		success: function (res) {
			jQuery('#loader').hide();
			jQuery('#payment_error').hide();
...
jQuery(document).on('submit', '#updateMyPassword', function(e){
  ...
  if(confirm_password !='' && new_password != '' && current_password !=''){
    if(new_password.length < 6){
      message = "<?php echo app('translator')->get('website.Please enter at least 6 characters'); ?>";
    ...
    if(confirm_password == new_password){
      ...
      message = "<?php echo app('translator')->get('website.New and confirm password does not match'); ?>";
      jQuery('#passowrd-error').text(message);
      jQuery('#passowrd-error').removeAttr('hidden');
      return false;
    }
    ...
    message = "<?php echo app('translator')->get('website.Please fill all the input fields'); ?>";
    jQuery('#passowrd-error').text(message);
    jQuery('#passowrd-error').removeAttr('hidden');
    return false;
  }
...
<script type="application/javascript">
	...
	OneSignal.push(function() {
		...
		OneSignal.getUserId(function(userId) {
			...
			jQuery.ajax({
				headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
				url: '<?php echo e(URL::to("/setSession")); ?>',
				type: "get",
	...
	jQuery.ajax({
		...
		success: function (res) {
			...
			if(res.remainingStock>0){
				...
				jQuery('#min_max_setting').html("<b><?php echo app('translator')->get('website.Min Order Limit:'); ?></b> &nbsp;" + min_level);
				jQuery('.stock-cart').removeAttr('hidden');
				jQuery('.stock-out-cart').attr('hidden',true);
				var max_order = jQuery('#max_order').val();
        ...
        jQuery('.variable-stock').text("<?php echo app('translator')->get('website.In stock'); ?>");
			}else{
				jQuery('#min_max_setting').html("");
				jQuery('.stock-out-cart').removeAttr('hidden');
				jQuery('.stock-cart').attr('hidden',true);
				...
				document.getElementById("myCheck").click();
        jQuery('.variable-stock').text("<?php echo app('translator')->get('website.Out of Stock'); ?>");
    ...
    if (tabCarousel.length) {
      tabCarousel.each(function () {
        ...
        thisCarousel.slick({
          ...
          //rtl:true,
          speed: 300,
          slidesToShow: item || <?php echo e($result['commonContent']['settings']['home_categories_records']); ?>,
          slidesToScroll: item || <?php echo e($result['commonContent']['settings']['home_categories_records']); ?>,
          adaptiveHeight: true,
          responsive: [{
            breakpoint: 1025,
            settings: {
...
<?php /**PATH K:\Freelancer\LAravel Ionic Fish\LaravelWebsite&AdminCode\resources\views/web/common/scripts.blade.php ENDPATH**/ ?>